...
std::vector<std::string> InferenceOutput::toCsvLines() {
    ...
    for(auto pl : networkProfile)
    {
        ...
        std::string line = networkName + sep + std::to_string(pl.layerId) + sep + pl.layerName + sep + std::to_string(pl.loadingProfile.duration) + sep + std::to_string(pl.ExecutionProfile.duration) + sep + policy;
