...
void NetworkProfiler::start(NetworkProfiler::LAYER_PROFILE_TYPE type) {
    switch (type) {
        case LAYER_PROFILE_TYPE::LOAD : currentLayer.loading.start();
        case LAYER_PROFILE_TYPE::EXEC : currentLayer.executing.start();
        case LAYER_PROFILE_TYPE::UNLOAD : currentLayer.unloading.start();
    }
}

void NetworkProfiler::end(NetworkProfiler::LAYER_PROFILE_TYPE type) {
    switch (type) {
        case LAYER_PROFILE_TYPE::LOAD : currentLayer.loading.stop();
        case LAYER_PROFILE_TYPE::EXEC : currentLayer.executing.stop();
        case LAYER_PROFILE_TYPE::UNLOAD : currentLayer.unloading.stop();
    }
}
...
std::string LayerProfile::toCsv() {
    ...
    return layerName + sep + std::to_string(layerId) + sep + std::to_string(loading.duration) + sep + std::to_string(executing.duration) + sep + std::to_string(unloading.duration) + sep + std::to_string(paramFileSize);
}

std::string LayerProfile::csvHeaders() {
    std::string sep = ",";
    return "layerName" + sep + "layerId" + sep + "loading_ns" + sep + "exec_ns" + sep + "unload_ns" + sep + "param_file_bytes";
}
