...
int main(int argc, char *argv[])
{
    ...
    std::string selectedNetwork = getArgs<std::string>(result, "network", "AgeNet", config, configAsText);
    /**
     * End parsing input parameters
     */

    EdgeCaffe::Orchestrator_old::MODEL_SPLIT_MODE mode = EdgeCaffe::Orchestrator_old::MODEL_SPLIT_MODE::PARTIAL;
    std::string modeAsString = "partial";
    if (defaultMode == "bulk")
    {
        mode = EdgeCaffe::Orchestrator_old::MODEL_SPLIT_MODE::BULK;
        modeAsString = "bulk";
    } else if (defaultMode == "deepeye")
    {
        mode = EdgeCaffe::Orchestrator_old::MODEL_SPLIT_MODE::DEEPEYE;
        modeAsString = "deepeye";
    } else if (defaultMode == "linear")
    {
        mode = EdgeCaffe::Orchestrator_old::MODEL_SPLIT_MODE::LINEAR;
        modeAsString = "linear";
    }
...
//    configAsText["generalOutputFile"] = generalOutputFile;
    ...
    EdgeCaffe::Orchestrator_old orchestrator;
...
//    double interArrivalTime = 20'000;
     ...
     */
    EdgeCaffe::Output output;
    std::string generalLine = memLimit + "," + modeAsString + "," + std::to_string(duration) + "," + selectedNetwork;
