...
def create_variation(exp: Experiment, memory: str, label_prob: float, mode: str, n_workers: int, path: str, batch_size : int):
    tag = f'{exp.experiment_tag}-{memory}-{label_prob}-{mode}-b{batch_size}'
    ...
    variation.batch_size = batch_size
        ...
        for label_prob in exp.success_label_prob:
            for mode in exp.modes:
                for n_worker in exp.n_workers:
                    for batch_size in exp.batch_size:
                        variation = create_variation(exp, mem, label_prob, mode, n_worker, path, batch_size)
...
def main():
    ...
    base_path = 'experiments/sipmc/batch'
    path = Path(base_path)
